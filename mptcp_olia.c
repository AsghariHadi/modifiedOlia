...
static void mptcp_olia_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
	...
	printk("Generated streamID: %u rtt is (Average of RTTs): %d Reordering: %d Current receiver window is:  %d Current snd_cwnd is: %d the max_window is %d",streamID, tp->srtt_us,tp->reord_seen, tp->rcv_wnd ,tp->snd_cwnd, max_cwnd );
