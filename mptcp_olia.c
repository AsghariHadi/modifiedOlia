...
static void mptcp_olia_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
	...
	printk("Total number of pkts sent during Recovery is: %d", tp->prr_out );
	printk("Total data packets lost incl. rexmits is: %d", tp->prr_out );
	printk("time we reached delivered is: %lld", tp->delivered_mstamp );
	printk("Current receiver window is: %d", tp->rcv_wnd );
	printk("Highest announced right edge  is: %d", tp->rcv_right_edge );
	printk("Lost packets is: %d", tp->lost_out );	
	printk("Total retransmits for entire connection is: %d", tp->total_retrans );	
